...
touch $tmpFile
# 运行pricirc
echo -n "running priCirc ......"
priCirc $priCircInputFile $tmpFile > /dev/null
if [ $2 -eq 1 ]
then
    time gringo $priCircFactFile $tmpFile | claspD 0
else
    time gringo $priCircFactFile $tmpFile | claspD 0 > /dev/null
fi

echo
# 运行circ2dlp
echo -n "running circ2dlp ......"
gringo $circ2dlpInputFile > $tmpFile
if [ $2 -eq 1 ]
then
    time circ2dlp $tmpFile -m "abx* : abo* : aba*" -v "ina* inb* out*" | claspD 0
else
    time circ2dlp $tmpFile -m "abx* : abo* : aba*" -v "ina* inb* out*" | claspD 0 > /dev/null
fi
